...
async function main() {
  ...
  try {
    ...
    assert.match(viewSource, /Удалить уборку/)
    assert.match(viewSource, /Будут удалены чек-лист, фотографии и ссылка клиента\. Это действие нельзя отменить\./)
    assert.match(viewSource, /router\.replace\('\/cleaner\/jobs'\)/)
